...
on_client_connected(#{client_id := ClientId, username := Username}, ConnAck, ConnInfo, ExchangeName) ->
  ...
  emqx_plugin_rabbitmq_cli:publish(ExchangeName, bson_binary:put_document(Doc), <<"client.connected">>),
  ok.
...
on_message_publish(Message = #message{topic = Topic, flags = #{retain := Retain}}, ExchangeName) ->
  ...
  emqx_plugin_rabbitmq_cli:publish(ExchangeName, bson_binary:put_document(Doc), <<"message.publish">>),
  {ok, Message}.
