...
connect(Opts) ->
  io:format("hook_rabbitmq_host: ~s~n", [proplists:get_value(hook_rabbitmq_host, Opts)]),
  ConnOpts = #amqp_params_network{
    host = proplists:get_value(hook_rabbitmq_host, Opts),
    port = proplists:get_value(hook_rabbitmq_port, Opts),
    username = proplists:get_value(hook_rabbitmq_username, Opts),
    password = proplists:get_value(hook_rabbitmq_password, Opts)
  },
